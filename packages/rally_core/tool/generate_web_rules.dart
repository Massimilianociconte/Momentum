...
///
/// Output: apps/padelandia-web/src/content/generated/padel-rules.json
///
...
Directory _repoRoot() {
  ...
  while (true) {
    if (Directory('${dir.path}/apps/padelandia-web').existsSync()) return dir;
    final parent = dir.parent;
    if (parent.path == dir.path) {
      throw StateError('Cartella apps/padelandia-web non trovata sopra $dir');
    }
    dir = parent;
  }
}
...
void main() {
  ...
  final document = {
    ...
    'generator': 'packages/rally_core/tool/generate_web_rules.dart',
    'note':
        'Generato da rally_core: non modificare a mano. La fonte unica è '
        'packages/rally_core/lib/src/rules/rules_data.dart.',
    'sections': sections,
  };

  final file = File(
    '${_repoRoot().path}/apps/padelandia-web/src/content/generated/'
    'padel-rules.json',
  );
