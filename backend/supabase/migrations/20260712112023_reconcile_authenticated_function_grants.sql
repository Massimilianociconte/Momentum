...
-- Reconcile historical remote grants. Some early migrations granted EXECUTE
-- directly to authenticated before their local definitions were hardened.
-- Revoking from every exposed role first makes the final allowlist independent
-- from the order in which an existing project originally applied migrations.
do $$
declare
  f regprocedure;
begin
